...
def untoggle_environment_objects(world, semantic_tags):
    """
    Untoggle the environment objects from the world, which are NOT specified in the semantic_tags.

    Args:
        world (carla.World)
        semantic_tags (list(carla.CityObjectLabel)): specifies the city object labels, which are NOT being toggled off
...
def change_color_texture_of_objects(world, filter_criteria, color, width, height, material):
    ...
    Example:
        change_color_texture_of_objects(world, filter_criteria='BP_StreetLight_6', color=carla.Color(r=255, g=255, b=255, a=255), width=100, height=100, material=carla.MaterialParameter.Diffuse)
    """
...
def get_texture(color, width, height):
    """
    creates the texture for the change_color_texture_of_objects() function
...
def spawn_prop(world, prop, transform):
    """
    ...
    Example:
        spawn_prop(world, 'static.prop.creasedbox03', carla.Transform(carla.Location(-25.5, 120.0, 0.0 ), carla.Rotation(0, 0, 0)))
    """

    bp = world.get_blueprint_library().find(prop)
    world.spawn_actor(bp, transform)

def change_vehicle_physics(vehicle, torque_curve=None, max_rpm=None, moi=None, damping_rate_full_throttle=None, damping_rate_zero_throttle_clutch_engaged=None,
    ...
    """
    change the physic of the vehicle by adjusting the wheels and e.g. changing the center of mass
    ...
    Example:
        change_vehicle_physics(vehicle=self._ego_vehicle, adjust_wheels=[1, 1, 0, 0], radii=[30.0, 100.0, 60.0, 40.0])

        change_vehicle_physics(vehicle=self._ego_vehicle, torque_curve=[carla.Vector2D(x=0, y=400), carla.Vector2D(x=1300, y=600)],
